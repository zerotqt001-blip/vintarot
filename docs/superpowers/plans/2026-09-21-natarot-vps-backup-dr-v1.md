...
- [ ] **Step 1: Write failing tests** for the helper's `VACUUM INTO` copy, WAL-safe reopen, integrity check, migration/table metadata, and missing-source failure.
- [ ] **Step 2: Run the focused test file** and confirm it fails because the helper does not exist.
- [ ] **Step 3: Implement the smallest Node helper** using `DatabaseSync`, escaped `VACUUM INTO`, read-only destination validation, and metadata-only output.
- [ ] **Step 4: Run the focused test file** and confirm all helper cases pass.
- [ ] **Step 5: Commit** `test: cover SQLite backup verification`.
...
- [ ] **Step 1: Add failing tests** for successful fixture backup, missing database, disk threshold failure, archive checksum, manifest exclusion of environment values, and lock overlap.
- [ ] **Step 2: Run the focused tests** and confirm the expected failures.
- [ ] **Step 3: Implement the runner** with strict mode, `flock`, stage trap, disk preflight, helper invocation, allowlisted configuration capture, manifest/checksum generation, tar verification, status markers, and fail-safe retention.
- [ ] **Step 4: Run the focused tests** and confirm success, then run `bash -n deploy/backup/natarot-backup.sh`.
- [ ] **Step 5: Commit** `ops: add verified VPS backup runner`.
...
- [ ] **Step 1: Add failing tests** for archive extraction, restored database validation, production-path isolation, missing archive failure, and no-provider read-only catalog smoke behavior.
- [ ] **Step 2: Run the focused tests** and confirm the expected failures.
- [ ] **Step 3: Implement isolated extraction, migration-runner validation, optional local server startup, HTTP catalog check, cleanup trap, and safe result logging.
- [ ] **Step 4: Run the focused tests** and confirm success.
- [ ] **Step 5: Commit** `ops: add isolated backup restore verification`.
...
- [ ] **Step 1: Add failing deployment-contract assertions** for daily and monthly timers, lock/failure logging, no product migration, and restore isolation.
- [ ] **Step 2: Run the focused contract test** and confirm it fails.
- [ ] **Step 3: Add the units and runbooks** with exact paths and commands, excluding all secrets and private key material.
- [ ] **Step 4: Run contract tests, Markdown placeholder scans, shell syntax checks, and `git diff --check`.
- [ ] **Step 5: Commit** `docs: document NaTarot backup and disaster recovery`.
...
- [ ] **Step 1: Re-run read-only production inventory** and record release, database, service, Nginx, disk, and firewall evidence.
- [ ] **Step 2: Install the helper, scripts, and units** with restrictive ownership/modes and run `systemctl daemon-reload`.
- [ ] **Step 3: Enable the backup and restore-test timers** without restarting `natarot.service`.
- [ ] **Step 4: Run one real backup** with `systemctl start --wait natarot-backup.service` and verify archive, manifest, checksum, database integrity, and status markers.
- [ ] **Step 5: Run the isolated restore test** and verify the restored database and loopback catalog response.
- [ ] **Step 6: Re-check production service, database file/WAL/SHM metadata, release marker, and schema counts** to prove production was not modified by restore testing.
...
- [ ] **Step 1: Create or locate the private Drive folder** without changing sharing permissions.
- [ ] **Step 2: Copy the verified archive to a local ephemeral path**, upload it, and verify remote filename, size, timestamp, and checksum metadata where available.
- [ ] **Step 3: Write only non-secret remote identity/checksum metadata to the VPS marker** and verify the marker has no credentials.
- [ ] **Step 4: Configure a quiet daily off-site sync task** that triggers only when the backup ID is new or remote verification is missing, and remains silent when unchanged.
- [ ] **Step 5: Test the sync task once** and document the exact off-site health check.
...
- [ ] **Step 1: Review as SRE, DBA, security engineer, release engineer, and operator.**
- [ ] **Step 2: Fix every Critical or Important finding and rerun focused tests/live verification.
- [ ] **Step 3: Run the final repository gates**: focused tests, tracked regression baseline as practical, TypeScript/build only if source tooling requires it, shell syntax, deployment contracts, secret scan, and `git diff --check`.
- [ ] **Step 4: Inspect staged diff for secrets, database files, archives, logs, and generated output.
- [ ] **Step 5: Commit `ops: add verified VPS backup and recovery workflow` and push `codex/natarot-vps-backup-dr-v1`.
- [ ] **Step 6: Verify local and remote branch heads match and produce the specified final report, then stop.
