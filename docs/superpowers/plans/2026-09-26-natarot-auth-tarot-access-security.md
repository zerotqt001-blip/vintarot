...
- [ ] **Step 7: Verify with a fresh, signed-out profile and public share.** Earlier clean Chrome Incognito checks confirmed Home, Guidebook, Practice, guest drawing and the login/register gate. On the current production release, unsigned reading, follow-up and clarification POSTs and saved-history GET returned 401 without provider calls; the POSTs also returned 401 with forged identity headers and an invalid session cookie. The Owner's saved reading has not yet been shared: action-time confirmation is pending before public exposure, so opening a share URL anonymously remains outstanding. Expired-session behavior and private owner-ID isolation are covered by the automated auth/session/security suite.
...
- [ ] **Step 9: Run manager cleanup only after browser verification.** Pre-cleanup checks on 2026-09-27 show three distinct references and a passing backup-retention/restore verifier. Cleanup has not run because the requested public-share browser check is still pending action-time confirmation. After that gate passes, use `NATAROT_BROWSER_VERIFIED=1 /usr/local/sbin/natarot-release-manager cleanup`; verify three releases remain and all backup hashes/inventory match the pre-cleanup snapshot. Preserve the unclassified `.incoming` and quarantined `.failed` roots.
