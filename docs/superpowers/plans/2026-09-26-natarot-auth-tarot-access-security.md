...
- [x] **Step 2: Run required repository gates.** Run `npx tsx --test tests/*.test.ts`, `npx tsc --noEmit`, `npm run build`, changed-file ESLint, and `git diff --check`. Record the clean production-branch baseline (647 passing, 11 pre-existing failing UI/migration assertions) separately; do not call a baseline failure a pass.
...
The focused auth, trusted-identity, Credits, Tarot API, Room-resume and UI suite passes `82/82`; TypeScript, production build, changed-file ESLint outside the legacy Room file, and `git diff --check` pass. The full suite is `666/677` with the same 11 failing assertions as the clean production baseline (`647/658`). Changed-file lint reports zero errors and one existing `window.location.assign` warning in `app/auth/auth.tsx`; `app/room/room.tsx` retains its baseline 23 errors and 6 warnings. No automated test calls an AI provider.

Because the full regression gate is red, this plan stops before archive creation, a fresh production backup, promotion, browser QA, Owner QA, and release cleanup. Production `current`, both rollback references, existing backups, and incoming release artifacts remain untouched. The registration return path is safe and same-tab; if email verification opens in a separate tab, the user must return to the original tab before signing in to resume the one-hour draft.
