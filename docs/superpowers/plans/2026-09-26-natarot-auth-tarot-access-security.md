...
- [x] **Step 6: Promote atomically with the release manager.** Release `tarot-ai-auth-security-69a1675-20260926T1234Z` was promoted from source commit `69a1675`; production later advanced through managed releases without a redeploy in this task. The service is active and health is `ok`. At the 2026-09-27 check, `current`, `previous-1`, and `previous-2` resolve to three distinct successful releases; the security release is now `previous-2`.

- [x] **Step 7: Verify a synthetic public share in an isolated, signed-out browser session.** The one existing saved Owner reading was private and was not shared. With no suitable synthetic reading available, exactly one fictional, non-sensitive QA reading was created under the authorized Owner QA account using one existing Credit (90 to 89 available, zero reserved), and the existing sharing feature created its public link. After signing out in the dedicated isolated in-app browser, the link returned HTTP 200 and rendered the question, all three card images, and the complete interpretation without login. The public page contained no Owner name/email, Credit/VIP/referral details, or AI/follow-up input. A cookie-free `POST /api/tarot/reading` with the synthetic session ID returned 401; the production unauthenticated path rejects before provider construction. No provider call was made during anonymous verification. The bearer link is not stored in this plan. Expired-session behavior and private owner-ID isolation are covered by the automated auth/session/security suite.

- [x] **Step 8: Verify Owner QA without changing account entitlements.** In the isolated Codex in-app browser, verified the existing Owner QA member and internal VIP entitlement, then created one genuine three-card reading, requested one AI interpretation using one existing Credit, saved it to the Owner journal, and confirmed it appears in reading history. A single authorized follow-up returned successfully without another Credit debit. Final balance is one Credit lower, with zero reserved; no account reset, payment, duplicate reading or retry occurred. This used one interpretation and one necessary follow-up provider call.

- [x] **Step 9: Run safe manager cleanup after browser verification.** On 2026-09-27, `NATAROT_BROWSER_VERIFIED=1 /usr/local/sbin/natarot-release-manager cleanup` completed with `retained=3 deleted=0`. Current, previous-1, and previous-2 still resolve to the same three distinct successful releases. `.incoming` was classified as unknown (15 directories and 7 files) and `.failed` as quarantined (3 entries); both were inspected and left untouched. `.staging` was empty. The backup verifier passed with 7 daily, 1 weekly, and 1 monthly archive; pre/post archive and checksum-sidecar SHA-256 inventories matched exactly. The final storage audit passed, `natarot.service` remained active, and local health returned `ok`.

- [x] **Step 10: Record final status and verify the docs checkpoint.** Updated `docs/PROJECT_STATE.md` and this plan with evidence from the anonymous share and cleanup. Only these documentation files are changed; no source deployment, test rerun, additional AI request, database archive or checksum-sidecar mutation, or unclassified-artifact deletion was performed. Commit, push, and remote equality are verified for this documentation checkpoint.
