...
- [x] **Step 2: Run required repository gates.** Run `npx tsx --test tests/*.test.ts`, `npx tsc --noEmit`, `npm run build`, changed-file ESLint, and `git diff --check`. All 677 tracked tests pass after refreshing 11 migration/UI test expectations to the current production source contracts; the same 11 assertions had already failed on the clean production baseline (`647/658`). ESLint comparison adds no findings: `app/room/room.tsx` remains at the baseline 23 errors and improved from 6 to 5 warnings; other changed files have zero errors and one existing auth navigation warning. Do not misstate those inherited Room findings as clean.
...
- [ ] **Step 5: Create a fresh production backup and verify checksum, readability, integrity, and restore-test status.** Stop if any backup or release-manager gate fails.

- [ ] **Step 6: Promote atomically with the release manager.** Restart only `natarot.service`, verify local and public health, and retain current plus two rollback references.

- [ ] **Step 7: Verify with a fresh, signed-out Chrome profile.** Exercise public pages, guest catalog/draw, UI login gate, direct valid-shaped guest API request (expect 401 before provider), forged/expired identity inputs, and public share read. Do not click a billable provider action.

- [ ] **Step 8: Verify Owner QA without changing account entitlements.** Confirm authenticated identity and current Credits/VIP state in a separate NaTarot-only browser context. Do not provision/reset the account, perform payment, or spend a production Credit. If a safe authenticated provider-free AI check is unavailable, report that limitation instead of claiming production AI generation was exercised.
...
The focused auth, trusted-identity, Credits, Tarot API, Room-resume and UI suite passes `82/82`; the full tracked suite passes `677/677`; TypeScript, production build, and `git diff --check` pass. The earlier 11 failing test assertions were stale contracts also failing on the clean production baseline (`647/658`); the assertions now reflect current source and retain their migration/UI coverage. Changed-file ESLint comparison adds no findings: the Room file retains its 23 baseline errors and has 5 warnings versus 6 at baseline; other changed files have zero errors and one existing `window.location.assign` warning. No automated test calls an AI provider.

The full regression gate is now green. Candidate archive creation, fresh production backup, promotion, browser QA, Owner QA, and cleanup remain outstanding. Production `current`, both rollback references, existing backups, and incoming release artifacts remain untouched. The registration return path is safe and same-tab; if email verification opens in a separate tab, the user must return to the original tab before signing in to resume the one-hour draft.
