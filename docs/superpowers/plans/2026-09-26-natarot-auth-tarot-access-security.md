...
- [ ] **Step 7: Verify with a fresh, signed-out profile and public share.** Earlier clean Chrome Incognito checks confirmed Home, Guidebook, Practice, guest drawing and the login/register gate. On the current production release, unsigned reading, follow-up and clarification POSTs and saved-history GET returned 401 without provider calls. The Owner's saved reading has not yet been shared: action-time confirmation is pending before public exposure, so opening a share URL anonymously remains outstanding. Expired-session behavior and private owner-ID isolation are covered by the automated auth/session/security suite.
