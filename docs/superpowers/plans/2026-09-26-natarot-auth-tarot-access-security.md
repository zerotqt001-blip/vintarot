...
- [ ] **Step 8: Verify Owner QA without changing account entitlements.** A fresh isolated Codex in-app browser tab is waiting at `/auth?mode=login&return_to=%2Froom`; it is separate from all Chrome profiles. The user must sign in there. Then confirm authenticated identity and current Credits/VIP state. The user authorized exactly one genuine Tarot reading and the existing sharing flow using current Owner QA entitlements; neither has happened yet. Do not provision/reset the account, perform payment, or make unnecessary provider requests.
