...
4. A root-only CLI runs against the fixed production SQLite path. It requires production mode, an explicit invocation flag, the exact member ID and normalized email, both Owner evidence references, an operator reference, and verified-backup references. It has no HTTP route.
