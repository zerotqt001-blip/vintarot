...
- Local backup remains owned by the existing `natarot-backup.service`, whose installed writer enforces limits of 7 daily, 4 weekly, and 3 monthly archives. The latest audit found 7 daily, 1 weekly, and 1 monthly archive currently populated. The existing archive contains a consistent SQLite snapshot, sidecar and manifest checksums, deployment/configuration metadata, and restore-test support; it is not encrypted before upload. Do not replace it or delete local backup data.
...
- Require the stored grant to include exactly the Drive scope `drive.file`; if the connection is missing it or includes a broader Drive scope, stop before making any Google API request.
...
- A separate root-owned systemd oneshot/timer reads only verified local archive references for offsite work, within the existing 7/4/3 retention limits. It encrypts each archive with AES-256-GCM using a per-archive data key wrapped by the existing NaTarot PII keyring under a distinct purpose; no new plaintext secret is stored. It uses Drive resumable upload sessions for archives and manifests, verifies remote ID/name/size/app properties/checksum, downloads and decrypts the remote copy, and runs the existing isolated restore verifier with a separate status root. The separate manifest remains `pending` until restore verification succeeds, then is updated to `verified`, downloaded again, and checked before recording the backup-set run as successful. Retry state is retained. It mirrors only references that currently exist (7 daily, 1 weekly, 1 monthly at audit time). After each new copy passes restore verification, only app-owned obsolete Drive files are moved to Trash. Local backups remain untouched.
- Prolonged offsite failures are recorded per complete backup-set run in the `System` tab and systemd journal. The durable run audit starts before Sheets synchronization so timeout recovery also detects runs interrupted during reporting. Success on an individual archive does not reset a failing run. Once a connected owner account exists, a single bilingual operational alert is sent to that account's verified Google email after a continuous 24-hour backup-set failure and at most once per 24 hours, using the existing configured Resend service. The alert contains only the failure category and last successful complete-set timestamp.
