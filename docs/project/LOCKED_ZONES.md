...
### Zone: Platform identity and ownership boundary

- **STATUS:** HARD LOCK
- **WHY PROTECTED:** This boundary controls authenticated identity, guest identity, cookie ownership, API authorization, saved-reading ownership, and room access.
- **FILES / MODULES:** `app/chatgpt-auth.ts`, `lib/request-identity.ts`, `lib/tarot-guest.ts`, `lib/server.ts`, `app/api/records/route.ts`, `app/api/rooms/route.ts`, `app/api/tarot/session/route.ts`, `app/api/tarot/saved-readings/route.ts`, `deploy/nginx/natarot-http.conf`.
...
- **REQUIRED TESTS:** `tests/f001-identity-boundary.test.ts`, `tests/request-identity.test.ts`, `tests/server-origin.test.ts`, affected API ownership tests, and the full suite.
...
- **FILES / MODULES:** `lib/ai/`, `lib/ai/prompts/`, `lib/ai/knowledge-v5.ts`, `natarot-knowledge/v5/`, `lib/tarot-reading-context.ts`, `lib/tarot-interpretation.ts`, `lib/tarot-reading-service.ts`, `lib/tarot-follow-up-service.ts`, reading API routes, `components/reading/`.
...
- **REQUIRED TESTS:** `tests/tarot-ai.test.ts`, `tests/tarot-interpretation.test.ts`, `tests/tarot-reading-context.test.ts`, `tests/tarot-reading-service.test.ts`, `tests/tarot-reading-route.test.ts`, `tests/tarot-follow-up*.test.ts`, saved-reading compatibility tests, typecheck, build.
...
- **FILES / MODULES:** `db/schema.ts`, `db/index.ts`, `drizzle/*.sql`, `drizzle/meta/`, `scripts/node-migrate.mjs`, `lib/runtime.ts`, `lib/sqlite-d1.ts`, `lib/tarot-repository.ts`, persistence API routes.
...
- **FILES / MODULES:** `.openai/hosting.json`, `vite.config.ts`, `scripts/run-framework.mjs`, `scripts/build-verified.sh`, `scripts/sites-env.*`, `deploy/nginx/`, `deploy/systemd/`, production environment files outside the repository.
...
### Zone: Home/mobile polish currently in working tree

- **STATUS:** ACTIVE
- **WHY PROTECTED:** The current checkout contains user-owned uncommitted changes in `app/vintarot.tsx`, `app/globals.css`, `docs/PROJECT_STATE.md`, and Home tests. They must not be overwritten or folded into governance work.
...
- **ALLOWED CHANGES:** Preserve exactly during this bootstrap. Future changes require the user's explicit scope and a fresh baseline.
...
Payment, video, email, public access, R2 storage, public reading share links, export/QR, Google Drive, membership, affiliate, and admin capabilities are not protected as implemented features because the inspected source does not provide those systems. Future work must first create a discovery record and decide whether each is planned, experimental, or out of scope.
