...
## Verification
`npx tsc --noEmit`, `npm run build`, `npx tsx --test tests/i18n.test.ts tests/shuffle.test.ts tests/shuffle-motion.test.ts tests/guidebook.test.ts` (9/9), and `git diff --check` pass for the locale, shuffle and Guidebook work. The production archive was validated and the first private Sites deployment succeeded. Browser checks covered English/Vietnamese switching, persistence across routes, Create topic suggestions, the continuous 3D-cylinder Room shuffle, two-tap stop/settle, fan draw, card flip, translated spread controls, Guidebook family selection and upright/reversed card details. Full fidelity and production integrations remain incomplete.
