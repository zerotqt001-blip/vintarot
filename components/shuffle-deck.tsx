...
export default function ShuffleDeck({active,paused=false,settling=false,onStart}:{active:boolean,paused?:boolean,settling?:boolean,onStart:()=>void}){
 return <button type="button" className={'shuffle-deck '+(active?'is-mixing':'')+(settling?' is-gathering':'')+(paused?' is-paused':'')} aria-label={active?(paused?'Resume shuffling':'Pause shuffling'):'Shuffle deck'} disabled={settling} onClick={onStart}>
