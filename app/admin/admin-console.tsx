  ...
  return (
    <div className="functional-page admin-functional-page">
      ...
      {detail && <section className="functional-section" aria-labelledby="admin-commerce-title">
        ...
        <div><h3>Referral attributions</h3>{affiliate?.attributions.length ? <div className="functional-table-wrap"><table><thead><tr><th>Attribution</th><th>Referrer member ID</th><th>Referred member ID</th><th>Attributed</th><th>Window ends</th></tr></thead><tbody>{affiliate.attributions.map((item) => <tr key={item.id}><td>{opaqueLabel(item.id)}</td><td>{item.referrerMemberId}</td><td>{item.referredMemberId ?? "Anonymous context"}</td><td>{dateLabel(item.attributedAt)}</td><td>{dateLabel(item.expiresAt)}</td></tr>)}</tbody></table></div> : <p className="functional-status">No referral attributions are available.</p>}</div>
      </section>}
