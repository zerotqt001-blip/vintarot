  ...
  return (
    <div className="functional-page admin-functional-page">
      ...
      {detail && <section className="functional-section" aria-labelledby="admin-commerce-title">
        ...
        <div className="functional-action-grid"><div><h3>User orders</h3><div className="functional-table-wrap"><table><thead><tr><th>Order</th><th>Status</th><th>Amount</th><th>Payment ref</th></tr></thead><tbody>{detail.orders.map((order) => <tr key={order.id}><td>{order.id}</td><td>{order.status}</td><td>{moneyLabel(order.amountMinor, order.currency)}</td><td>{order.paymentReferenceMasked ?? "—"}</td></tr>)}</tbody></table></div></div><div><h3>User affiliate state</h3>{detail.affiliate ? <><p>{detail.affiliate.profile ? `${detail.affiliate.profile.id} · ${detail.affiliate.profile.status}` : "No affiliate profile"}</p><small>{detail.affiliate.summary.conversions} conversions · {moneyLabel(detail.affiliate.summary.netMinor, "VND")} net commission</small><div className="functional-table-wrap"><table><thead><tr><th>Conversion</th><th>Status</th><th>Commission</th></tr></thead><tbody>{detail.affiliate.history.slice(0, 8).map((item) => <tr key={item.id}><td>{item.id}</td><td>{item.status}</td><td>{moneyLabel(item.commissionMinor, item.currency)}</td></tr>)}</tbody></table></div></> : <p className="functional-status">Affiliate state is restricted for this staff role.</p>}</div></div>
