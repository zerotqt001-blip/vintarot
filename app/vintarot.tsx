...
import { AlignLeft, ArrowRight, CalendarDays, Camera, CirclePlay, CircleUserRound, Crown, Gift, Globe2, Heart, HelpCircle, Home, Layers, Moon, Music2, Play, Search, Share2, ShoppingBag, Sparkles, Star, UserRound } from "lucide-react";
...
function VinTarotShell({ user, children, path }: { user: User; children?: React.ReactNode; path: string }) {
  ...
  const shellClass = isHome ? "home-shell" : isMembership ? `membership-shell${membershipTheme === "soft" ? " membership-shell-soft" : ""}` : isRoom ? "site-shell room-shell" : isGuidebook ? "site-shell guidebook-shell" : isCreate ? "site-shell create-shell" : isPractice ? `site-shell practice-shell${practiceTheme === "soft" ? " practice-shell-soft" : ""}` : isDaily ? "site-shell daily-shell" : "site-shell";
  return <SidebarProvider><ReferralCapture enabled={Boolean(user)} /><div ref={shellRef} data-home-atmosphere={homeAtmosphere} className={shellClass}>
    ...
    <header className="topbar">
      ...
      </div> : isHome ? <div className="top-actions home-header-actions">
    ...
    {!isHome && !isRoom && !isPractice && !isMembership && <nav className="commerce-access-nav commerce-access-nav--desktop" aria-label={`${t("nav.membership")} / ${t("nav.affiliate")} / ${t("nav.account")}`}>
      {renderCommerceAccessLinks()}
    </nav>}
    {!isHome && !isRoom && !isPractice && !isMembership && <nav className="commerce-access-nav commerce-access-nav--mobile" aria-label={`${t("nav.membership")} / ${t("nav.affiliate")} / ${t("nav.account")}`}>
      {renderCommerceAccessLinks()}
    </nav>}
    <Sidebar collapsible="none" className="site-sidebar"><SidebarContent>
      ...
      })}</nav> : isMembership ? <nav className="main-nav" aria-label={t("nav.membership")}>{membershipNav.map(([key, Icon, href]) => <a className={href === path ? "active" : ""} href={href} key={href} aria-label={t(key)} aria-current={href === path ? "page" : undefined}><span className="nav-orb"><Icon size={29} strokeWidth={1.3} /></span><ArcLabel id={`membership-nav-arc-${href.replace(/[^a-z0-9]+/gi, "-")}`} text={t(key)} /><span className="mobile-nav-label">{t(key)}</span></a>)}</nav> : <nav className="main-nav">{renderSitePrimaryNav()}</nav>}
      {isPractice ? <div className="practice-v1-sidebar-signoff"><span>NaTarot</span><small>{t("nav.innerLight")}</small></div> : isMembership ? <div className="membership-sidebar-signoff"><span>NaTarot</span><small>Find Your Inner Light</small></div> : !isHome && <nav className="personal-nav"><a className="username" href={profileHref}>{user?.username || t("nav.yourSpace")}</a>{personal.map(([key, Icon, href]) => <a key={href} href={href}><Icon size={17} strokeWidth={1.3} />{t(key)}</a>)}</nav>}
    ...
    {!isHome && !isPractice && !isMembership && <button className="help" aria-label={t("header.help")} onClick={() => setModal("help")}><HelpCircle size={23} strokeWidth={1} /></button>}
