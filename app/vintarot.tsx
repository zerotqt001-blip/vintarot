...
function VinTarotShell({ user, children, path }: { user: User; children?: React.ReactNode; path: string }) {
  ...
  return <SidebarProvider><ReferralCapture enabled={Boolean(user)} /><div ref={shellRef} data-home-atmosphere={homeAtmosphere} data-guidebook-theme={isGuidebook ? guidebookTheme : undefined} className={shellClassName}>
    ...
    {!isHome && !isRoom && !isGuidebook && !isAffiliate && !isMembership && <nav className="commerce-access-nav commerce-access-nav--desktop" aria-label={`${t("nav.membership")} / ${t("nav.affiliate")} / ${t("nav.account")}`}>
      {renderCommerceAccessLinks()}
    </nav>}
    {!isHome && !isRoom && !isGuidebook && !isAffiliate && !isMembership && <nav className="commerce-access-nav commerce-access-nav--mobile" aria-label={`${t("nav.membership")} / ${t("nav.affiliate")} / ${t("nav.account")}`}>
      {renderCommerceAccessLinks()}
    </nav>}
    ...
    {!isHome && !isGuidebook && !isMembership && <button className="help" aria-label={t("header.help")} onClick={() => setModal("help")}><HelpCircle size={23} strokeWidth={1} /></button>}
