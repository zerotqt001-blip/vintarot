...
function createPreShareDatabase(dbPath: string): void {
  ...
  const migrations = readdirSync(migrationDirectory)
    .filter((name) => /^\d{4}_.+\.sql$/.test(name) && !name.startsWith("0005_") && !name.startsWith("0006_") && !name.startsWith("0007_"))
    .sort();
...
test("Node migration bootstrap applies and repeats the full schema and seed", (t) => {
  ...
  assert.equal((sqlite.prepare("SELECT COUNT(*) AS count FROM natarot_migrations").get() as { count: number }).count, 10);
  assert.deepEqual(
